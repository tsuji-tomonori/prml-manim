...
低品質版は約 **9分59秒（854×480 / 15 fps）**。**WhiteCUL の音声と同文字幕付き** です。

## シーン

| Scene | 内容 | 目安 | 見るもの |
|---:|---|---:|---|
| 1 | この点を生んだ曲線は？ | 51.1秒 | 点だけの問い、隠れた sin、未知の観測 |
| 2 | 多項式はつまみの集まり | 67.5秒 | w₀…w₃ の縦スライダーと式の色分け |
| 3 | ずれを面積に変える | 79.5秒 | 符号付き和／二乗和、残差→正方形→E の棒 |
| 4 | 二つのつまみで谷底へ | 56.1秒 | 誤差の等高線、係数の点、直線が連動 |
| 5 | 点を通れば予測もよくなる？ | 105.8秒 | M=0…9、残差から訓練／テスト RMS を点打ち |
| 6 | 曲線の裏側で係数が膨らむ | 56.0秒 | 正負の係数を対数の棒で比較 |
| 7 | 点が増えると同じ九次式は？ | 50.1秒 | M=9、N=10→15→40→100 を同じ軸で比較 |
| 8 | 係数をゼロへ引くバネ | 83.5秒 | ln λ を往復、曲線・棒・RMS が連動 |
| 9 | 一本の線のその先へ | 49.7秒 | 同じ入力の繰り返し観測、既知ノイズの帯 |

台本の各 beat にナレーション兼字幕、実測尺と原文参照があります。
...
構文確認、数値・音声管理の5検証、キャッシュ無効のフルレンダリング、ffprobe、無音・音量検査を実施しました。
最終動画は映像 599.332678 秒（H.264 / 854×480 / 15 fps）、音声 599.360000 秒（AAC）です。
3秒以上の無音は0件、平均音量 -26.6 dB、ピーク -5.6 dB。
各シーン最低2枚を含む35フレームを目視し、シーン2・3・5・8の音声文境界・PCM発声開始・動作時刻を照合しました。
全編の通し聴取と全フレームの目視は未実施です。
詳細は [今回の作業完了レポート](../../../reports/working/20260925-1630-prml-1-1-voicevox-sync.md) を参照してください。
前回の無音版の記録は [前回レポート](../../../reports/working/20260925-1501-prml-1-1-3b1b-remake.md) に残します。
