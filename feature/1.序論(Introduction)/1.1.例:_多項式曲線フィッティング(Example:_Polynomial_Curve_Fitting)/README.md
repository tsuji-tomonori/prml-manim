...
構文確認、既存の5テスト、160文の修正前後の audio_query、字幕安全領域、キャッシュ無効のフルレンダリングを確認しました。
字幕 display と timeline に読み方のカタカナは0件。記号入り18枚を含む最終42フレームを目視し、3シーンの文境界・PCM発声開始・動作時刻を照合しました。
最終動画は映像 600.066344 秒、音声 600.106667 秒（差 0.040323 秒）。
3秒以上の無音は0件、平均音量 -26.6 dB、ピーク -5.7 dB。
全編の通し聴取と全フレームの目視は未実施です。
詳細は [今回の作業完了レポート](../../../reports/working/20260925-2131-prml-1-1-caption-readings.md) を参照してください。
