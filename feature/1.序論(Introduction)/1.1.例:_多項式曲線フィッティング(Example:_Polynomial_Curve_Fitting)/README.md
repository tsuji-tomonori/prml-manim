...
低品質版は約 **11分35秒（854×480 / 15 fps）**。現在は **音声未再生成の字幕付き無音版** です。

## シーン

| Scene | 内容 | 目安 | 見るもの |
|---:|---|---:|---|
| 1 | この点を生んだ曲線は？ | 57秒 | 点だけの問い、隠れた sin、未知の観測 |
| 2 | 多項式はつまみの集まり | 80秒 | w₀…w₃ の縦スライダーと式の色分け |
| 3 | ずれを面積に変える | 91秒 | 符号付き和／二乗和、残差→正方形→E の棒 |
| 4 | 二つのつまみで谷底へ | 61秒 | 誤差の等高線、係数の点、直線が連動 |
| 5 | 点を通れば予測もよくなる？ | 120秒 | M=0…9、残差から訓練／テスト RMS を点打ち |
| 6 | 曲線の裏側で係数が膨らむ | 60秒 | 正負の係数を対数の棒で比較 |
| 7 | 点が増えると同じ九次式は？ | 67秒 | M=9、N=10→15→40→100 を同じ軸で比較 |
| 8 | 係数をゼロへ引くバネ | 105秒 | ln λ を往復、曲線・棒・RMS が連動 |
| 9 | 一本の線のその先へ | 54秒 | 同じ入力の繰り返し観測、既知ノイズの帯 |

台本の各 beat にナレーション、字幕、尺と原文参照があります。
...
- `assets/voicevox/manifest.json`: 台本と WAV のハッシュ、生成状態、beat ごとの尺。
...
2026-09-25 の確認時点で `http://127.0.0.1:50021` に接続できず、音声は未再生成です。
旧 13 シーンの WAV は削除し、9 シーンの manifest を `pending` にしました。
WAV がない、台本とハッシュが合わない、WAV が破損した場合、映像は無音でレンダリングします。
尺は台本文字数（毎秒6文字＋余白）と演出の最低時間から決めます。
...
音声は beat ごとに長さを測って余白を付け、Manim の各動作と同じ境界に揃えます。
単に WAV ファイル名が一致するだけでは、音声を採用しません。
`--prepare-only` は Engine に接続せず、古い音声の無効化と manifest の整合だけを行います。

ナレーション：**VOICEVOX:WhiteCUL**（style ID 23）。実音声での聴取・無音区間・音量の検証は再生成後に必要です。
...
構文確認、数値・音声管理の5検証、キャッシュ無効のフルレンダリング、ffprobe、代表25フレームの目視を実施しました。
最終動画は 694.667 秒、H.264、854×480、15 fps、音声ストリームなしです。
全フレームの人手による総当たり検査と、実音声の合成・同期・音量確認は未実施です。
詳細は [作業完了レポート](../../../reports/working/20260925-1501-prml-1-1-3b1b-remake.md) を参照してください。
